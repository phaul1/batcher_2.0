...
# Read transaction details from the user once
echo
...
async function main() {
    ...
    for (const txDetails of transactions) {
        for (let i = 0; i < txDetails.numberOfTransactions; i++) {
            ...
            // Generate a random delay between 0 and 48 hours
            const delay = Math.floor(Math.random() * 48 * 60 * 60 * 1000);
            console.log(\`Waiting for \${delay / 1000 / 60} minutes before sending the next transaction...\`);
